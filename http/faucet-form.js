...
export class FaucetForm extends BaseElement {
  ...
  render() {
    ...
    return html`
      <div class="message">
        Enter your address and the amount of Spectre you want to receive:
      </div>
      <flow-input
      ...
      ></flow-input>
      <flow-input label="Amount (SPR)" class="amount" value=""></flow-input>
      <flow-select
        label="Network"
      ...
      >
        ${this.networks.map(
          (n) =>
            html`<flow-menu-item value="${n}">${aliases[n]}</flow-menu-item>`,
        )}
      </flow-select>
      <div class="captcha">
  ...
  submit() {
    ...
    //spectretest:qq0nvlmn07f6edcdfynt4nu4l4r58rkquuvgt635ac
    console.log({ address, network, amount, captcha });

    if (!/^spectre:[1-9A-HJ-NP-Za-km-z]/.test(address)) {
      ...
      (error, result) => {
        ...
        if (error) {
          ...
          if (error.error == "limit") {
            ...
            msg = html`Unable to send funds: you have
              <b>${SPR(available)}</b> SPR
              ${period == null
                ? html`available.`
                : html`remaining.<br />&nbsp;<br />Your limit will update in
                    ${FlowFormat.duration(period)}.`}`;
          } else {
            msg = error.error || error.toString();

            if (/ApiError/.test(msg)) {
              msg = html`<div class="api-error">
                ${[msg].map((v) => html`${v}<br />`)}
              </div>`;
            }
          }
        ...
        FlowDialog.show({
          ...
          body: html`
            <div class="msg">
              ...
              <b>${SPR(result.amount)} SPR</b> to the requested address:<br />&nbsp;<br />
              <b>${address}</b><br />&nbsp;<br />
