      ...
      .then((data) => {
        ...
        if (errors && errors.length) {
          if (errors.includes("invalid-input-response")) {
            error =
              req && req._T
                ? req._T("Please complete captcha")
...
class SpectreFaucet extends EventEmitter {
  constructor(appFolder) {
    super();
    this.appFolder = appFolder;
    this.config = utils.getConfig(
      path.join(appFolder, "config", "spectre-faucet"),
    );
    if (!this.config.apikeys) log.error("missing config.apikeys configuration");
...
*/
  async initSpectre() {
    ...
    if (this.options.rpc && filter.length != 1) {
      log.error(
        "You must explicitly use the network flag when specifying the RPC option",
      );
    ...
    for (const { network, port } of Object.values(Wallet.networkTypes)) {
      ...
      this.limits[network] =
        this.options.limit === false
          ...
          : Decimal(this.options.limit || 1000)
              .mul(1e8)
              .toNumber(); // || limits_[network] || 1000;
      this.wallets[network].setLogLevel(log.level);

      log.info(
        `${Wallet.networkTypes[network].name} address - ${this.addresses[network]}`,
      );
    }
  ...
  calculateAvailable({ network, ip, address }) {
    ...
    const transactions = (user[network] = user[network].filter(
      (tx) => tx.ts > period_start,
    ));
    ...
    const period = transactions.length
      ? transactions[0].ts - period_start
      : null;
    log.info(
      `Address ${address} on ip ${ip} has ${available / 100000000} SPR available`,
    );
  ...
  async initFaucet() {
    ...
    (async () => {
      for await (const msg of requests) {
        ...
        let captchaRes = {};
        captchaRes = await validatehCaptcha({
          captcha,
          secret: this.config.captcha.secret,
        }).catch((err) => {
        ...
        log.info(`request from ${ip} for ${Wallet.SPR(amount)}`);
        const limit =
          this.limits[network] === false
            ? Number.MAX_SAFE_INTEGER
            : this.limits[network] || 0;
        ...
        let { available, period } = this.calculateAvailable({
          network,
          ip,
          address,
        });
        if (available < amount) {
          ...
          try {
            ...
            ({ available, period } = this.calculateAvailable({
              network,
              ip,
              address,
            }));
    ...
    const getAvailable = async ({ network, ip }) => {
      if (!network || !/^spectre(test|dev|sim)*/.test(network))
    ...
    const getSpectre = async ({ address, amount: amount_, ip }) => {
      const amount = parseInt(amount_);
      if (isNaN(amount) || !amount || amount < 0)
        return { error: `Invalid amount: ${amount_}` };

      if (!address || !/^spectre(test|dev|sim)*:/.test(address))
      ...
      if (!this.wallets[network])
        return {
          error: `Wallet interface is not active for network ${network}`,
        };
      ...
      if (available < amount) {
        return {
          error: `Unable to send funds: you have ${this.SPR(available)} SPR ${period == null ? `available.` : `remaining. Your limit will update in ${this.duration(period)}.`}`,
        };
      } else {
        try {
          const fee = 0;
    ...
    /*
		...
		this.flowHttp.app.get("/api/:apikey/get/:address", async (req,res) => {
			...
			const ip = req.query.ip;
			res.json(await getSpectre({address, amount, ip}))
		});
		*/
    ...
    for (const [network, wallet] of Object.entries(this.wallets)) {
      ...
      wallet.on("balance-update", (detail) => {
        ...
        flowHttp.sockets.publish(`balance`, {
          network,
      ...
      wallet.on("utxo-change", (detail) => {
        ...
        flowHttp.sockets.publish(`utxo-change`, {
          network,
          added,
          removed,
          seq: seq++,
  ...
  async main() {
    ...
    program
      ...
      .option(
        "--log <level>",
        `set log level ${logLevels.join(", ")}`,
        (level) => {
          if (!logLevels.includes(level))
            throw new Error(
              `Log level must be one of: ${logLevels.join(", ")}`,
            );
          return level;
        },
      )
      ...
      .option(
        "--port <port>",
        `set http port (default ${this.options.port})`,
        (port) => {
          port = parseInt(port);
      ...
      .option("--limit <limit>", `SPR/day limit per IP`, (limit) => {
        limit = parseFloat(limit);
        if (isNaN(limit) || limit <= 0)
          throw new Error("SPR/day limit is invalid");
        return limit;
      })
      .option("--no-limit", "disable SPR/day limit")
...
(async () => {
  let spectreFaucet = new SpectreFaucet(__dirname);
  try {
    await spectreFaucet.main();
  } catch (ex) {
